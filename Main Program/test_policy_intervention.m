function test_policy_intervention()
    clc; clear; close all;
    fprintf('Initializing Policy Intervention Test\n');
    ...
    fprintf('Configuring target parameters...\n');
    
    TargetParams = struct();
    
    %Change policy parameters here
    ...
    TargetParams.tau_c_s = -0.20;  
    TargetParams.policy_end_t = T_periods; 
    
    fprintf('Target Policies: Tau_E_0=%.1f%%, Tau_int=%.1f%%\n', ...
        TargetParams.tau_e_0*100, TargetParams.tau_int*100);
    ...
    if ~isempty(FinalRes)
        ...
        format_val = @(v) strrep(sprintf('%.1f', v * 100), '.', 'p');
        
        file_name = sprintf('Policy_tauE_%s_gE_%s_tauInt_%s.mat', ...
                            format_val(TargetParams.tau_e_0), ...
                            format_val(TargetParams.g_tau_e), ...
                            format_val(TargetParams.tau_int));
...
function visualize_all(Base, Final, BasePol, TargetPol, T)
    ...
    subplot(2,2,1); plot(time, Share_Cm_base, 'k--', 'LineWidth', 1.5); hold on; plot(time, Share_Cm_pol, 'b-', 'LineWidth', 2); title('Consumption Share: Manufacturing'); legend('Baseline', 'Policy'); grid on; ylim([0 1]);
    subplot(2,2,2); plot(time, Share_Cs_base, 'k--', 'LineWidth', 1.5); hold on; plot(time, Share_Cs_pol, 'r-', 'LineWidth', 2); title('Consumption Share: Services'); legend('Baseline', 'Policy'); grid on; ylim([0 1]);
    ...
    subplot(2,3,1); plot(time, Base.I, 'k--', 'LineWidth', 1.5); hold on; plot(time, Final.I, 'g-', 'LineWidth', 2); title('Investment Level (I)'); legend('Baseline', 'Policy'); grid on;
    subplot(2,3,2); plot(time, Base.E, 'k--', 'LineWidth', 1.5); hold on; plot(time, Final.E, 'b-', 'LineWidth', 2); title('Nominal Expenditure (E)'); legend('Baseline', 'Policy'); grid on;
    subplot(2,3,3); plot(time, Base.p_m ./ Base.p_s, 'k--', 'LineWidth', 1.5); hold on; plot(time, Final.p_m ./ Final.p_s, 'm-', 'LineWidth', 2); title('Relative Price (Pm / Ps)'); legend('Baseline', 'Policy'); grid on;
    ...
    subplot(1,3,1); plot(time, Base.K, 'k--', 'LineWidth', 1.5); hold on; plot(time, Final.K, 'b-', 'LineWidth', 2); title('Capital Stock (K)'); legend('Baseline', 'Policy'); grid on; xlabel('Time');
    subplot(1,3,2); plot(time, Base.R, 'k--', 'LineWidth', 1.5); hold on; plot(time, Final.R, 'r-', 'LineWidth', 2); title('Resource Stock (R)'); legend('Baseline', 'Policy'); grid on; xlabel('Time');
    subplot(1,3,3); plot(time, Base.R_input, 'k--', 'LineWidth', 1.5); hold on; plot(time, Final.R_input, 'm-', 'LineWidth', 2); title('Extraction Flow (R_{input})'); legend('Baseline', 'Policy'); grid on; xlabel('Time');
    set(findobj(gcf, 'Type', 'axes'), 'XLim', [0, obs_period]);
